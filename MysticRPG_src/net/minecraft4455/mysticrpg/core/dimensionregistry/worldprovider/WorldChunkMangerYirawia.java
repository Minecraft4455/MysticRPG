...
import net.minecraft4455.mysticrpg.core.dimensionregistry.genlayer.GenLayerYirawia;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class WorldChunkMangerYirawia extends WorldChunkManager {
	private GenLayer myGenBiomes;
	private GenLayer myBiomeIndexLayer;
	private BiomeCache myBiomeCache;
	private List<BiomeGenBase> myBiomesToSpawnIn;

	protected WorldChunkMangerYirawia() {
		this.myBiomeCache = new BiomeCache(this);
		this.myBiomesToSpawnIn = new ArrayList<BiomeGenBase>();

		this.myBiomesToSpawnIn.add(MysticBiomes.YirawiaPlains);

	}

	public WorldChunkMangerYirawia(long seed, WorldType worldtype) {
		this();
		GenLayer[] agenlayer = GenLayerYirawia.makeTheWorld(seed);
		this.myGenBiomes = agenlayer[0];
		this.myBiomeIndexLayer = agenlayer[1];
	}

	public WorldChunkMangerYirawia(World world) {
	...
	public BiomeGenBase getBiomeGenAt(int x, int z) {
		...
		if (biome == null) {
			return MysticBiomes.YirawiaPlains;
		}
	...
	public BiomeGenBase[] getBiomesForGeneration(
		...
		for (int i = 0; i < par4 * par5; ++i) {
			if (aint[i] >= 0) {
				...
				par1ArrayOfBiomeGenBase[i] = MysticBiomes.YirawiaPlains;
			}
		}
	...
	public boolean areBiomesViable(int par1, int par2, int par3,
			@SuppressWarnings("rawtypes") List par4List) {
		IntCache.resetIntCache();
		int l = par1 - par3 >> 2;
		int i1 = par2 - par3 >> 2;
		int j1 = par1 + par3 >> 2;
