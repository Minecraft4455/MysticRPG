...
public class YirawiaTeleporter extends Teleporter {
	...
	public YirawiaTeleporter(WorldServer par1WorldServer) {
				...
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						this.worldServerInstance.setBlock(k1, l1, i2,
								flag ? MysticBlocks.YirawiaBlock.blockID : 0);
					}
				}
			}
			...
			for (k1 = l - short1; k1 <= l + short1; ++k1) {
				...
				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
					...
					for (int i2 = this.worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2) {
						/** change this block **/
						if (this.worldServerInstance.getBlockId(k1, i2, l1) == MysticBlocks.YirawiaPortal.blockID) {
							/** change this block **/
							while (this.worldServerInstance.getBlockId(k1,
									i2 - 1, l1) == MysticBlocks.YirawiaPortal.blockID) {
								--i2;
							}
		...
		if (d3 >= 0.0D) {
			if (flag) {
				this.destinationCoordinateCache.add(j1,
						new YirawiaPortalPosition(this, i, j, k,
			...
			d4 = (double) k + 0.5D;
			int j2 = -1;
			if (this.worldServerInstance.getBlockId(i - 1, j, k) == MysticBlocks.YirawiaPortal.blockID) {
				j2 = 2;
			}
			if (this.worldServerInstance.getBlockId(i + 1, j, k) == MysticBlocks.YirawiaPortal.blockID) {
				j2 = 0;
			}
			if (this.worldServerInstance.getBlockId(i, j, k - 1) == MysticBlocks.YirawiaPortal.blockID) {
				j2 = 3;
			}
			if (this.worldServerInstance.getBlockId(i, j, k + 1) == MysticBlocks.YirawiaPortal.blockID) {
				j2 = 1;
		...
		for (k2 = 0; k2 < 4; ++k2) {
			for (i3 = 0; i3 < 4; ++i3) {
				for (l2 = -1; l2 < 4; ++l2) {
					...
					this.worldServerInstance.setBlock(k3, j3, i4,
							flag ? MysticBlocks.YirawiaBlock.blockID
